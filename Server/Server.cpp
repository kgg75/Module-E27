...
#define CONSOLE_MUTEX_NAME consoleMutex		//	для макроса CONSOLE_CRITICAL_SECTION
...
void AtStart() {	
	...
	logger << (char*)BuildStatement(SEPARATOR, GetTime().c_str(), " сервер запущен");
}
...
inline void ZeroRequestBuffer() { WIN(memset(request_msg, 0, BUFFER_LENGTH)) NIX(bzero(request_msg, BUFFER_LENGTH)); }	//	очмстка буфера для отправки данных
inline void ZeroResponseBuffer() { WIN(memset(response_msg, 0, BUFFER_LENGTH)) NIX(bzero(response_msg, BUFFER_LENGTH)); }	//	очмстка буфера для приёма данных
	...
	switch (command_code) {
			...
			if (iResult > 0) {
				...
				if (response_msg[0] != ERR_USER_ALREADY_LOGGED) {
					...
					if (iResult > 0) {	// login прошёл успешно
						...
						/*CONSOLE_CRITICAL_SECTION(
							cout << '\n' << GetTime() << " на сокете #" << connection_descriptor << " вошёл в чат пользователь '" << login << "'\n";
							ConsoleWaitMessage();
						)*/
						logger << (char*)BuildStatement(GetTime().c_str(), " на сокете #", to_string(connection_descriptor).c_str(), " вошёл в чат пользователь '", login.c_str(), "'");
					}
				}
			}
		...
		case LCMD_USER_DELETE:
			...
			if (iResult == MSG_SUCCESS_RESULT) {
				...
				GetLogin(users_map.at(connection_descriptor), login);
				logger << (char*)BuildStatement(GetTime().c_str(), " удалил учётную запись пользователь '", login.c_str(), "'");
			}
		...
		case SLCMD_CHANGE_PASSWORD:
			if (command_code == SLCMD_CHANGE_PASSWORD)
				iResult = SetPassword(users_map.at(connection_descriptor), (SHA1PwdArray&)request_msg[1]);
			else
			...
			if (response_msg[0] == MSG_SUCCESS_RESULT) {
				GetLogin(users_map.at(connection_descriptor), login);
				logger << (char*)BuildStatement(GetTime().c_str(), " изменил учётные данные пользователь '", login.c_str(), "'");
			}
			break; 
		case LCMD_LOGOUT:	// локальная команда logout - выход
			...
			logger << (char*)BuildStatement(GetTime().c_str(), " на сокете #", to_string(connection_descriptor).c_str(), " вышел из чата пользователь '", &request_msg[1], "'");
			break;
		case LCMD_IS_USER_EXIST:
			response_msg[0] = (char)(GetUserId(&request_msg[1]) > 0 ? MSG_SUCCESS_RESULT : ERR_USER_NOT_FOUND);
			break;
	...
	while (loop) {
		...
		if (!command.empty()) {
			switch (get_command_code(SERVER_COMMANDS, command)) {	// анализируем серверную команду
				case SCMD_CON:
					if (!users_map.empty()) {
						CONSOLE_CRITICAL_SECTION(cout << "Список подключений:\n   сокет : учётное имя\n";)
						for (map <SOCKET, uint>::iterator it = users_map.begin(); it != users_map.end(); it++) {
							if (it->second != 0)
							...
							CONSOLE_CRITICAL_SECTION(cout << "     " << it->first << " : " << text << "\n";)
						}
					}
					else {
						CONSOLE_CRITICAL_SECTION(cout << "Нет подключений.\n";)
					}
					break;
				case SCMD_USERS:
					iResult = PrepareListByOneU();
					...
					if (iResult > 0) {
						...
						auto i = 1;
						while (ListByOneU(i++, text) != ERR_NO_RESULT) {
							CONSOLE_CRITICAL_SECTION(cout << text;)
						}
					}
					else if (iResult == ERR_REQUEST_ERROR) {
						CONSOLE_CRITICAL_SECTION(PrintErrorMessage(iResult);)
					}
					else {
						CONSOLE_CRITICAL_SECTION(cout << "Нет пользователей.\n";)
					}
					break;
				case SCMD_LOG:
					CONSOLE_CRITICAL_SECTION(
					...
					break;
				case SCMD_HELP:
					CONSOLE_CRITICAL_SECTION(print_help(SERVER_COMMANDS);)
					break;
				case SCMD_EXIT:
					CONSOLE_CRITICAL_SECTION(
						bResult = (!users_map.empty() && QuestionYN("Есть действующие подключения. Всё равно закрыть сервер?")) || QuestionYN("Подтвердите остановку сервера");
					)
					if (bResult) {
						loop = false;	// инициализируем остановку сервера
						logger << (char*)BuildStatement(GetTime().c_str(), " сервер остановлен");
					}
					break;
				default:
					cout << MSG_ERR_CMD;
	...
	while (loop) {	// Communication Establishment
		...
		if (iResult > 0 && users_map.size() < FD_SETSIZE) {
			...
			if (socket WIN(== INVALID_SOCKET)NIX(< 0)) {
				...
				logger << (char*)BuildStatement(GetTime().c_str(), " Сервер не может принимать данные от клиента. Код ошибки: ", to_string(WSAGetLastError()).c_str());
			}
			else {
				users_map.emplace(socket, 0);
				logger << (char*)BuildStatement(GetTime().c_str(), " подключился клиент #", to_string(socket).c_str());
			...
			if (iResult WIN(== SOCKET_ERROR)NIX(!= 0)) {
				...
				logger << (char*)BuildStatement(GetTime().c_str(), " Внутренняя ошибка сервера. Код ошибки: ", to_string(WSAGetLastError()).c_str());
			}
		...
		for (map <SOCKET, uint>::iterator it = users_map.begin(); it != users_map.end(); ) {
			...
			if (iResult > 0) {
				...
				logger << (char*)BuildStatement(GetTime().c_str(), " Ошибки в подключении клиента ", to_string(it->first).c_str(), " Код ошибки: ", to_string(WSAGetLastError()).c_str());
			}
			...
			if (iResult > 0) {
				...
				if (iResult > 0) {
					if (strcmp(request_msg, CLIENT_STOP_MESSAGE) == 0) {
						logger << (char*)BuildStatement(GetTime().c_str(), " отключился клиент #", to_string(it->first).c_str());
					...
					//	ConsoleWaitMessage();
					logger << (char*)BuildStatement(GetTime().c_str(), " потеряна связь с клиентом #", to_string(it->first).c_str());
					it = users_map.erase(it);
					continue;
				}
			}
