...
int PrepareListByOneU() {
	...
	sqlResult = SQLExecDirectA(sqlStmtHandle, BuildStatement(
		"SELECT ",
			...
			user.Email.columnName, ",",
			user.RegDate.columnName, ","
			" ifnull(date_format(", user.LastVisit.columnName, ",", SQL_DATETIME_FORMAT, "),", SQL_LASTVISIT_NULL_TEXT, ")"
		" FROM ",
			USERS_TABLE_NAME,
		" WHERE ",
	...
	if (sqlResult == SQL_SUCCESS && col_count == 7 && row_count > 0) {	// 7 - число столбцов в выполненном выше запросе
		...
		sqlResult = SQLBindCol(sqlStmtHandle, 7, user.LastVisit.targetType, user.LastVisit.value, sizeof(SQL_LASTVISIT_NULL_TEXT), nullptr);	// длиннее, чем SQL_DATETIME_FORMAT!
...
int ListByOneU(const uint user_index, string& output) {
	SQLRETURN sqlResult = SQLFetch(sqlStmtHandle);
