...
void PrintMessages(const uchar command_code, const string& login) {
	...
	do {
		...
		if (response_msg[0] == MSG_SUCCESS_RESULT) {
			CONSOLE_CRITICAL_SECTION(cout << &response_msg[1];)
		}
...
void RequestHandler() {
	...
	switch (command_code) {
		case LCMD_RECEIVE:
			...
			iResult =	WIN(send(server_descriptor, response_msg, response_msg_length, 0))
...
int ConsoleCommandsHandler() {	
	...
	while (loop) {
		ZeroRequestBuffer();
		auto g_command_code = get_command(GLOBAL_COMMANDS);

		switch (g_command_code) {	// получаем глобальную команду с консоли
			case GCMD_REG:	// команда reg - зарегистрироваться
				CONSOLE_CRITICAL_SECTION(
					bResult = NewUser(name, surname, login, email, password);
				)
				if (bResult) {
					index = 1;
			...
			case GCMD_LOGIN:	// команда login - вход
				...
				if (bResult) {
					...
					if (response_msg[0] == MSG_SUCCESS_RESULT) {	// пользователь успешно авторизовался
						CONSOLE_CRITICAL_SECTION(cout << "Пользователь '" << login << "' вошёл в чат.\n";)

						uint unReadedMsgs = RequestMessagesQty(login, LCMD_UNREADED_COUNT);
						if (unReadedMsgs > 0) {
							CONSOLE_CRITICAL_SECTION(
								cout << "Есть непрочитанные сообщения (" << unReadedMsgs << "). ";
								bResult = QuestionYN("Показать?");
							)
							if (bResult)	// вывод непрочитанных сообщений
						...
						while (!login.empty()) {	// пока пользователь не вышел из чата
							...
							//CONSOLE_CRITICAL_SECTION(
								auto l_command_code = get_command(LOCAL_COMMANDS, console_prompt);	// получаем локальную команду с консоли
							//)
							switch (l_command_code) {
									...
									if (response_msg[0] == MSG_SUCCESS_RESULT) {
										if (receiver != login) {
											...
											if (bResult) {
												...
												if (response_msg[0] != 0) {
													CONSOLE_CRITICAL_SECTION(cout << "Сообщение для '" << receiver << "' отправлено.\n";)
												}
											}
										} else {
											CONSOLE_CRITICAL_SECTION(cout << "Нельзя отправлять сообщения самому себе.\n";)
										}
									}
									else {
										CONSOLE_CRITICAL_SECTION(PrintErrorMessage(response_msg[0]);)
									}
									break;
								case LCMD_READ: {	// локальная команда read - прочитать сообщения
										...
										if (messsagesQty > 0)
											...
											CONSOLE_CRITICAL_SECTION(cout << "Нет " << (answer ? "непрочитанных " : "") << "сообщений.\n";)
										}
									}
									break;
								...
								case LCMD_USER_DELETE:	// локальная команда delete - удалить учётную запись пользователя
									CONSOLE_CRITICAL_SECTION(
										bResult =	QuestionYN("Информация о пользователе будет навсегда удалена. Вы уверены?") &&
								...
								case LCMD_USER_CHANGE: {	// локальная команда change - изменить учётные данные пользователя
										...
										while (lcmd_loop) {
											...
											request_msg_length = 1;	// устанавливаем длину сообщения по умолчанию

											//CONSOLE_CRITICAL_SECTION(
												auto sl_command_code = get_command(SUBLOCAL_COMMANDS, console_prompt);	// получаем локальную команду с консоли
											//)
											switch (sl_command_code) {
												...
												case SLCMD_CHANGE_PASSWORD:	// локальная команда pwd		- изменить пароль пользователя
													CONSOLE_CRITICAL_SECTION(
														bResult = SetRegData(any_text, sl_command_code);
													)
													if (!bResult)
														break;
												...
												case SLCMD_HELP:	// локальная команда help - вывести справку по командам
													CONSOLE_CRITICAL_SECTION(print_help(SUBLOCAL_COMMANDS);)
													break;
												case SLCMD_QUIT:	// локальная команда quit - выйти из этого меню
													lcmd_loop = false;
													CONSOLE_CRITICAL_SECTION(cout << "Вы вышли из меню изменения учётных данных.\n\n";)
													break;
												default:
													CONSOLE_CRITICAL_SECTION(cout << MSG_ERR_CMD;)
											}
										}
								...
								case LCMD_HELP:	// локальная команда help - вывести справку по командам
									CONSOLE_CRITICAL_SECTION(print_help(LOCAL_COMMANDS);)
									break;
			...
			case GCMD_HELP:	// команда help - вывести справку по командам
				CONSOLE_CRITICAL_SECTION(print_help(GLOBAL_COMMANDS);)
				break;
